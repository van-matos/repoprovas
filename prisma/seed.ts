...
async function main() {
  await connection.term.createMany({
    data: [
      { number: 1 },
      { number: 2 },
      { number: 3 },
  ...
  await connection.teacher.createMany({
    data: [{ name: "Diego Pinho" }, { name: "Bruna Hamori" }],
    skipDuplicates: true,
  });

  await connection.discipline.createMany({
    data: [
  ...
  await connection.teacherDiscipline.createMany({
    data: [
